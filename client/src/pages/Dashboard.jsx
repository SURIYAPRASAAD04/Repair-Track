...
export default function Dashboard() {
  ...
  const handleConnectWhatsApp = async () => {
    if (isMobile) {
      // On mobile, show method picker (QR vs Pairing Code)
      setShowMethodPicker(true);
      return;
    }
    // Desktop: go straight to QR
    await startQRFlow();
  };
  ...
  const handlePairingCodeConnect = () => {
    setShowMethodPicker(false);
    setShowPairing(true);
  };
  ...
  return (
    <div className="space-y-6 sm:space-y-8 animate-in fade-in duration-300 pb-20 md:pb-8 relative">
      ...
      {/* Mobile connection method picker */}
      {showMethodPicker && (
        <div className="fixed inset-0 bg-black/80 backdrop-blur-sm flex items-end justify-center z-[100] animate-in fade-in duration-200">
          <div className="bg-surface-card border-t border-surface-border rounded-t-2xl w-full overflow-hidden animate-in slide-in-from-bottom duration-300">
            ...
            <div className="p-4 space-y-3">
              ...
              <button
                onClick={handlePairingCodeConnect}
              ...
              >
                ...
                <div>
                  ...
                  <p className="text-xs text-text-muted mt-0.5">Get a 6-digit code — no second device needed</p>
                </div>
                <span className="ml-auto text-xs bg-accent-green/20 text-accent-green px-2 py-0.5 rounded-full font-medium">Recommended</span>
              </button>
              <button
                onClick={startQRFlow}
              ...
              >
                ...
                <div>
                  ...
                  <p className="text-xs text-text-muted mt-0.5">Use another device to scan the code</p>
                </div>
              </button>
            </div>
            <div className="h-6" />
